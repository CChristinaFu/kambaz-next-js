...
export default function WorkingWithArrays() {
  ...
  return (
    <div id="wd-working-with-arrays">
      ...
      <a id="wd-update-todo-title" className="btn btn-primary float-end" href={`${API}/${todo.id}/title/${todo.title}`}>
        Update Todo Title
      </a>
      ...
      <h4>Updating the Description</h4>
      <a id="wd-update-todo-description" className="btn btn-primary float-end" href={`${API}/${todo.id}/description/${todo.description}`}>
      ...
      <h4>Updating the Completed Status</h4>
      <a id="wd-update-todo-completed" className="btn btn-primary float-end" href={`${API}/${todo.id}/completed/${todo.completed}`}>
        Update Completed of Todo ID = {todo.id}
      </a>
      <FormControl defaultValue={todo.id} className="w-25 float-start me-2"
        onChange={(e) => setTodo({ ...todo, id: e.target.value })} />
      <div className="form-check float-start mt-2">
        <input type="checkbox" id="wd-todo-completed" className="form-check-input"
          checked={todo.completed}
          onChange={(e) => setTodo({ ...todo, completed: e.target.checked })} />
        <label htmlFor="wd-todo-completed" className="form-check-label ms-1">
          Completed
        </label>
      </div>
      <br /><br /><hr />
